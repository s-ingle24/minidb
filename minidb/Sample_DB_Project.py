    ...
    if pro == 1:
        while True:
            ...
            while True:
                ...
                if cursor.fetchone():
                    print("ID already exists. Please enter a different User ID.")
                else:
                    break  

            new_pass = input("Create Password[upto 16 characters]")
            
            #insert inputed data
            cursor.execute("""INSERT INTO Users (User_ID, Name, Email, Contact_No, Password)
            VALUES (%s,%s,%s,%s,%s)
            """,(id,user,mail,no,new_pass))
            trv.commit()
            print("sign in successful!")
            
        
            #show inserted data
            cursor.execute("""SELECT User_ID, Name, Email, Contact_No FROM Users
            WHERE User_ID = %s 
            """,(id,))
            signin = cursor.fetchall()
            headers=["ID","User Name","Email","Contact"]
             
            print(tabulate(signin, headers=headers, tablefmt="grid"))
            return True
    ...
    if pro == 2:
        while True:
            ...
            if log:
                ...
                print("Invalid. check user credentials")
                continue
